...
import ProfileModal from "../components/team/ProfileModal.jsx"; // default
import { MemberForm } from "../components/team/MemberForm"; // named
import { MembersTable } from "../components/team/MembersTable"; // named
...
export default function Dashboard() {
  ...
  const load = useCallback(
    async (query = "") => {
      ...
      setError("");
      try {
        const { data } = await teamService.search({
          nombre: query,
          perPage,
          sort: "latest",
        });
  ...
  // limpiar objectURL cuando cambie/desmonte
  useEffect(() => {
    return () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const fields = { ...form };
      delete fields.foto_url; // lo resuelve el server
      const payload = { ...fields, ...(fotoFile ? { foto: fotoFile } : {}) };

      if (editingSlug) {
        const { data: updated } = await teamService.update(
          editingSlug,
          payload
        );
        setItems((prev) =>
          prev.map((it) => (it.slug === editingSlug ? updated : it))
        );
      } else {
        const { data: created } = await teamService.create(payload);
        if (
          !search ||
          created.nombre.toLowerCase().includes(search.toLowerCase())
        ) {
  ...
  const onEdit = (m) => {
    ...
    if (m.foto_url) {
      ...
      setFotoPreview(
        m.foto_url.startsWith("http") ? m.foto_url : API_ORIGIN + m.foto_url
      );
    }
    document
  ...
  // ===== Tabs extra (placeholders) =====
  const CarruselTab = () => (
    <section className="rounded-xl border bg-[hsl(var(--card))] border-[hsl(var(--border))] p-6">
      <CarouselManager />
    </section>
  );
  ...
  const tabs = [
    { id: "miembros", label: "Miembros", element: <MiembrosTab /> },
  ...
  return (
    <div className={cx("p-6 space-y-6", pageClass)}>
      <header>
        ...
        <p className="text-[hsl(var(--fg))/0.8]">
          Estás dentro. Email: {user?.email}
        </p>
      </header>
