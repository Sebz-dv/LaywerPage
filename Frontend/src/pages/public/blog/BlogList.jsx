...
export default function BlogList({
  // ya NO usamos asset local; esto permite override desde fuera si algún día quieres
  heroImage, 
  ...
  // ✅ URL real del slot publication_hero
  const [heroImageUrl, setHeroImageUrl] = useState(null);

  // ================== Cargar imagen del slot ==================
  useEffect(() => {
    (async () => {
      try {
        const slot = await mediaService.getByKey("publication_hero").catch(
          () => null
        );
        if (slot?.url) {
          setHeroImageUrl(resolveAssetUrl(slot.url));
        } else {
          // si no hay slot, podemos usar heroImage (si viene de props) o nada
          setHeroImageUrl(heroImage || null);
        }
      } catch (e) {
        // eslint-disable-next-line no-console
        console.warn("No se pudo cargar media slot publication_hero:", e);
        setHeroImageUrl(heroImage || null);
      }
    })();
  }, [heroImage]);
  ...
  const effectiveHeroImg = heroImageUrl; // si no hay slot, puede ser null → hero sólo con color + overlays

  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 pb-10">
      {/* ===================== HERO FULL-BLEED ===================== */}
      <section
        className="w-screen h-[56svh] md:h-[64svh] relative overflow-hidden left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] mt-[-12px] z-0 bg-[hsl(var(--primary))]"
        aria-label="Cabecera visual de publicaciones"
      >
        {/* Fondo con blur + ken burns (sólo si hay imagen en el slot) */}
        {effectiveHeroImg && (
          <motion.img
        ...
        {/* Overlays para contraste (funcionan con o sin imagen) */}
        ...
        <div className="relative h-full grid place-items-center pointer-events-none">
            ...
            <div className="sm:col-span-2 lg:col-span-3">
              <div className="card card-pad text-center">
                ...
                {debouncedQ && (
                  <button
                    type="button"
                    className="btn btn-outline mt-3 touch-manipulation select-none"
                    onClick={() => {
                      setQ("");
                      setPage(1);
                    }}
                  ...
                  <div className="card-pad flex flex-col gap-2">
                    ...
                    {(it.pdf_url || it.external_url) && (
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        {it.pdf_url ? (
                          <a
                            className="btn btn-outline pointer-events-auto select-none"
                            href={it.pdf_url}
                            target="_blank"
                            rel="noreferrer"
                          >
        ...
        {meta && (
          <div className="mt-8 flex items-center justify-center gap-2">
            <button
              ...
              className="btn btn-outline disabled:opacity-50 touch-manipulation select-none"
